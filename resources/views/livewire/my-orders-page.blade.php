              ...
              <tbody>
                ...
                @php
                    ...
                    if($order->status == 'delivered'){
                        $status = '<span class="px-3 py-1 text-white bg-green-800 rounded shadow">Delivered</span>';
                    }
                    ...
                    if($order->payment_status == 'pending'){
                        $payment = '<span class="px-3 py-1 text-white bg-blue-500 rounded shadow">Pending</span>';
                    }

                    if($order->payment_status == 'paid'){
                        $payment = '<span class="px-3 py-1 text-white bg-green-500 rounded shadow">Paid</span>';
                    }
                    if($order->payment_status == 'failed'){
                        $payment = '<span class="px-3 py-1 text-white bg-red-500 rounded shadow">Failed</span>';
                    }
                @endphp
